class AvailabilityManager {
    ...
    buildSlotTable (slots) { // Depends on slots, setupDOM
        const createSlot = (item, index) => {
            ...
            const handleButtonClick = (button) => { // Function to handle button click event
                ...
                this.postSlotData(button.data('id'))
                    .then(response => {
                        debugger
                        this.formatColors(button, {table: "slot", label: response.status})
                    })
            ...
            this.formatColors(button, {table: "slot", label: item.status}) // Update the button color based on the slot status
    ...
    formatColors(elementProp, designation){
        ...
        if (designation.table === "slot") {
            if (designation.label === "available"){
                element.addClass('btn-warning')
                element.removeClass('btn-primary')
            } else {
                element.addClass("btn-primary")
    ...
    async getCalendar() { // Depends on getDate
        const date = this.getDate()
        const options = {
            year: 'numeric',
            month: '2-digit',
    ...
    handleDateChange (ev) {
        ...
        getSlotData // ok
        postSlotData // ok
        buildMonthYearSelector // ok
        buildSlotTable // ok
        buildCalendarTable // ok

        // Function to build calendar table
        const buildCalendarTable = (data) => {
            console.log(data)
            const date = getDate()
            // Clear the contents of the HTML container
            $('#Availability-Date').empty();
                    ...
                    const createSlot = (item, index) => {
                        ...
                        const handleButtonClick = (button) => { // Function to handle button click event
                            ...
                            updateSlotData(button.data('id'))
                                .then(response => {
                                    this.formatColors(button, {table: "slot", label: response.status}) // Update the button color based on the slot status
                                })
                                .catch(err => console.log(err));
                        };
                        // Execution
                        const tableBody = $('#Availability-Time-Table-Body') // created below
                        if (index % 2 === 0) { // Add a new row to the table body every two items
                            createTableRow().appendTo(tableBody);
                        }
                        const row = tableBody.children().last(); // Get the last row added to tbody
                        ...
                        this.formatColors(button, {table: "slot", label: item.status}) // Update the button color based on the slot status
                        // handleSlotColorUpdate(button, item.status);
                        createTableCell(button).appendTo(row); // Create a table cell with the button and append it to the row
                    }
                    const timeContainer = $('#Availability-Time')
                    const dateStr = currentButton.data('date');
        ...
        retrieveAvailabilityData(ev)
            .then(data => {
                buildCalendarTable('#gridContainer', data) // getCalendar
            })
    ...
    init(){
        $(document).ready(() => {
            ...
            this.setupDOM()
            $('#id_user').parent().on('change', this.buildUI.bind(this));
        });
    }
}
